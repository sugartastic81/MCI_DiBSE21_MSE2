...
void Ware::printWare() {
    string bezeichnung = this->getBezeichnung();
    ...
    std::cout << "\tSNR " << this->getSeriennummer() << "    \tw " << this->getGewicht() << " kg\t BP " << this->getEinkaufspreis() << " EUR\t\tSP " << this->getVerkaufspreis() << " EUR" << std::endl;
}